...
unsigned char* cuda_pixels = NULL;
// Init image data
void initBitmap(int width, int height)
{
	...
	cudaMalloc((void**)&cuda_pixels, width * height * 4);
	gImageWidth = width;
	gImageHeight = height;
}
...
__device__
int mandelbrot( int x, int y, int width, int height, int maxiter,
...
__global__
void computeFractal( unsigned char *ptr, int width, int height, int maxiter,
							MYFLOAT offsetx, MYFLOAT offsety, MYFLOAT scale)
{
    ...
    int fractalValue = mandelbrot( x, y, width, height, maxiter, offsetx, offsety, scale);
...
void Draw()
{
  ...
  cudaEventCreate(&mySecondEvent);
	...
	computeFractal<<<dimGrid, dimBlock>>>(cuda_pixels, gImageWidth,gImageHeight, maxiter, offsetx, offsety, scale);
cudaThreadSynchronize();
	...
	cudaMemcpy(pixels, cuda_pixels, 4*gImageWidth*gImageHeight, cudaMemcpyDeviceToHost);

	cudaEventDestroy(myEvent);
	cudaEventDestroy(mySecondEvent);
